...
transaction = pd.read_csv('tbl_transaction.csv')
...
# Sidebar navigasi
st.sidebar.image("logo_omron_remove.png", width=150)
...
if page == "Main Page":
            ...
            with button_col2:
                if st.session_state.get('scan_result'):
                    if st.button("Confirm Transaction", key="confirm_transaction_dialog"):
                        ...
                        stocklist.to_csv('tbl_stock.csv', index=False)
                        st.session_state.transaction.to_csv('tbl_transaction.csv', index=False)
    ...
    with col1:
        if st.button("Take Product", use_container_width=True, icon="⏫"):
            ...
            def takeout_product():
                ...
                # Check if image exists in images folder
                       ...
                       with button_col1:
                        if st.button("Cancel", key="cancel_dialog_take"):
                            st.rerun()
                       with button_col2:
    ...
    with col2:
        if st.button("Add Product", use_container_width=True, icon="⏬"):
    ...
    # Membuat tanggal awal dan akhir bulan yang dipilih
        ...
        for _, item_row in items_with_input.iterrows():
            ...
            try:
                ...
                # Cek apakah item ini adalah barang baru (status 'new')
                ...
                # Jika item baru
                if is_new_item:
                    initial_qty = 0  # Barang baru selalu dimulai dengan qty 0
                    total_input = pd.to_numeric(item_data['qty input'], errors='coerce').sum()
                    total_output = pd.to_numeric(item_data['qty output'], errors='coerce').sum()

                # Jika bukan barang baru, kita ambil transaksi sebelumnya
                else:
                    ...
                    if not previous_transactions.empty:
                        ...
                        # Jika tidak ada transaksi sebelumnya, ambil dari stocklist
                ...
                # Menghitung total input dan output bulan ini
